...
function calculate(saveHistory=true){if(!expression)return;try{const original=expression,v=evaluate(expression);lastResult=v;if(saveHistory)addHistory(original,format(v));expression=editableNumber(v);cursorPos=expression.length;render()}catch(e){toast(e.message||'Nieprawidłowe działanie')}}
