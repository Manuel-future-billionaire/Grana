...
const defaults={history:[],retention:0,precision:8,liveResult:true,thousandsSeparator:true,appearance:'system',haptics:false,animations:true,version1:true};
...
function load(){
  try{
    ...
    migrated.haptics=Boolean(parsed.haptics??false);
...
function bindLongPress(el){let t,sx,sy,longTriggered=false;const stop=()=>{clearTimeout(t);t=null};el.addEventListener('pointerdown',e=>{sx=e.clientX;sy=e.clientY;longTriggered=false;t=setTimeout(()=>{longTriggered=true;openMenu(el.dataset.id,e.clientX,e.clientY)},280)});el.addEventListener('pointermove',e=>{if(Math.hypot(e.clientX-sx,e.clientY-sy)>9)stop()});el.addEventListener('pointerup',()=>{stop();if(!longTriggered)useEntry(el.dataset.id)});el.addEventListener('pointercancel',stop);el.addEventListener('contextmenu',e=>{e.preventDefault();openMenu(el.dataset.id,e.clientX,e.clientY)})}
function openMenu(entryId,x,y){selectedId=entryId;const m=$('#contextMenu');m.classList.remove('hidden');const w=m.offsetWidth,h=m.offsetHeight;m.style.left=Math.max(8,Math.min(innerWidth-w-8,x-w/2))+'px';m.style.top=Math.max(8,Math.min(innerHeight-h-8,y-10))+'px';haptic(8)}
function closeMenu(){$('#contextMenu').classList.add('hidden')}
function openDisplayMenu(x,y){const m=$('#displayMenu');m.classList.remove('hidden');const w=m.offsetWidth,h=m.offsetHeight;m.style.left=Math.max(8,Math.min(innerWidth-w-8,x-w/2))+'px';m.style.top=Math.max(8,Math.min(innerHeight-h-8,y-10))+'px';haptic(8)}
...
async function menuAction(a){const x=entry();if(!x)return;if(a==='use')useEntry(x.id);else if(a==='copy'){await copy(`${x.expression} = ${historyResult(x)}\n${dateText(x.created)}${x.comment?'\nNotatka: '+x.comment:''}`)}else if(a==='note')showDialog('Notatka do obliczenia',x.comment,'note');else if(a==='edit')showDialog('Edytuj zapisane obliczenie',x.expression,'edit');else if(a==='delete'&&confirm('Usunąć ten wpis?')){state.history=state.history.filter(e=>e.id!==x.id);save();renderHistory();haptic(12)}closeMenu()}
...
function saveDialog(){const x=entry();if(!x)return;const value=$('#dialogText').value.trim();if(dialogMode==='note')x.comment=value;else{try{x.expression=extractExpression(value);x.resultValue=evaluate(x.expression);x.result=format(x.resultValue)}catch{toast('Nieprawidłowe działanie');return}}x.updated=Date.now();save();renderHistory();haptic(8)}
async function copy(text){try{await navigator.clipboard.writeText(text)}catch{const ta=document.createElement('textarea');ta.value=text;document.body.append(ta);ta.select();document.execCommand('copy');ta.remove()}haptic(8);toast('Skopiowano do schowka')}
...
$('#retention').value=state.retention;$('#precision').value=state.precision;$('#appearance').value=state.appearance;$('#liveResult').checked=state.liveResult;$('#thousandsSeparator').checked=state.thousandsSeparator;$('#haptics').checked=state.haptics;$('#animations').checked=state.animations;
$('#retention').onchange=e=>{state.retention=Number(e.target.value);cleanOld();renderHistory()};$('#appearance').onchange=e=>{state.appearance=e.target.value;save();applyPreferences()};$('#precision').onchange=e=>{state.precision=Number(e.target.value);save();render();renderHistory()};$('#liveResult').onchange=e=>{state.liveResult=e.target.checked;save();render()};$('#thousandsSeparator').onchange=e=>{state.thousandsSeparator=e.target.checked;save();render();renderHistory()};$('#haptics').onchange=e=>{state.haptics=e.target.checked;save();haptic(12)};$('#animations').onchange=e=>{state.animations=e.target.checked;save();applyPreferences()};
$('#exportBtn').onclick=()=>{const blob=new Blob([JSON.stringify({version:'1.0',exported:new Date().toISOString(),history:state.history},null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='kalkulator-kantor-historia.json';a.click();URL.revokeObjectURL(a.href)};
$('#importBtn').onclick=()=>$('#importFile').click();$('#importFile').onchange=async e=>{try{const obj=JSON.parse(await e.target.files[0].text());if(!Array.isArray(obj.history))throw 0;state.history=obj.history.map(x=>{const raw=Number.isFinite(Number(x.resultValue))?Number(x.resultValue):parseStoredResult(x.result);return {...x,expression:extractExpression(x.expression),resultValue:Number.isFinite(raw)?raw:undefined,locked:undefined}});save();renderHistory();toast('Historia zaimportowana')}catch{toast('Nieprawidłowy plik')}};
...
applyPreferences();save();cleanOld();render();renderHistory();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>{});
